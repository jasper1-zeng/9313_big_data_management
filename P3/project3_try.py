...
class Project3:
    ...
    def generate_prefixes_and_cells(self, record, s_threshold, d_threshold):
        ...
        prefix_length = min(prefix_length, terms_len)  # Ensure prefix_length does not exceed terms_len
    ...
    def format_output(self, record):
        """
        Formats the output by removing trailing zeros and unnecessary decimal points.
        Example:
            (0,2):1.414214, 0.75
            (2,3):1, 0.5
        """
    ...
    def trim_trailing_zeros(self, num):
        """
        Converts a float to a string, removing trailing zeros and the decimal point if not needed.
        """
        return ('{0:.6f}'.format(num)).rstrip('0').rstrip('.') if '.' in '{0:.6f}'.format(num) else '{0:.6f}'.format(num)
